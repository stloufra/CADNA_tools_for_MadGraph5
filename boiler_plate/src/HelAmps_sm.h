...
#define ALWAYS_INLINE
          ...
          const int ipar )        // input: particle# out of npar
  {
    ...
    if( fmass != 0. )
    {
      ...
      const fptype_sv pp = fpmin( pvec0, fpsqrt( pvec1 * 
                                                      pvec1 + 
                                                      pvec2 * 
                                                      pvec2 + 
                                                      pvec3 * 
                                                      pvec3 ) );
#else
      volatile fptype_sv p2 = pvec1 * 
                                   pvec1 + 
                                   pvec2 * 
                                   pvec2 + 
                                   pvec3 * 
                                   pvec3; // volatile fixes #736
      ...
      const int ip = ( 1 + 
                        nh ) / 
                        2; // NB: same as in Fortran pp==0, differs from Fortran pp>0, which is (3+nh)/2 because omega(2) has indexes 1,2
      const int im = ( 1 - 
                        nh ) / 
                        2; // NB: same as in Fortran pp==0, differs from Fortran pp>0, which is (3-nh)/2 because omega(2) has indexes 1,2
#ifndef MGONGPU_CPPSIMD
      ...
      else
      {
        const fptype sf[2] = { fptype( 1 + 
                               ...
                               fptype( 1 + 
                                        ...
                                        nh ) * 
                                        (fptype)0.5 };
        fptype omega[2] = { fpsqrt( pvec0 + 
                                         pp ), 0. };
        omega[1] = fmass / 
                        omega[0];
        const fptype sfomega[2] = { sf[0] * 
                                         omega[ip], sf[1] * 
                                         omega[im] };
        const fptype pp3 = fpmax( pp + 
                                    pvec3, 0. );
        const cxtype chi[2] = { cxmake( fpsqrt( pp3 * 
                                                   (fptype)0.5 / 
                                                   pp ), 0. ),
                                ( pp3 == 0. ? cxmake( -nh, 0. ) : cxmake( nh * 
                                                                            pvec1, pvec2 ) / 
                                                                            fpsqrt( 2. * 
                                                                            pp * 
                                                                            pp3 ) ) };
      ...
      const cxtype fiA_2 = ip * 
                             sqm[ip];                  // scalar cxtype: real part initialised from fptype, imag part = 0
      const cxtype fiA_3 = im * 
                             nsf * 
                             sqm[ip];            // scalar cxtype: real part initialised from fptype, imag part = 0
      const cxtype fiA_4 = ip * 
                             nsf * 
                             sqm[im];            // scalar cxtype: real part initialised from fptype, imag part = 0
      const cxtype fiA_5 = im * 
                             sqm[im];                  // scalar cxtype: real part initialised from fptype, imag part = 0
      // Branch B: pp != 0.
      const fptype sf[2] = { fptype( 1 + 
                                      nsf + 
                             ...
                             fptype( 1 + 
                                      ...
                                      nh ) * 
                                      (fptype)0.5 };
      fptype_v omega[2] = { fpsqrt( pvec0 + 
                                         pp ), 0 };
      omega[1] = fmass / 
                      omega[0];
      const fptype_v sfomega[2] = { sf[0] * 
                                         omega[ip], sf[1] * 
                                         omega[im] };
      const fptype_v pp3 = fpmax( pp + 
                                    pvec3, 0 );
      ...
      volatile fptype_v chi0r2 = pp3 * 
                                    0.5 / 
                                    ppDENOM;              // volatile fixes #736
      ...
      const cxtype_v fiB_2 = sfomega[0] * 
                                       chi[im];
      const cxtype_v fiB_3 = sfomega[0] * 
                                       chi[ip];
      const cxtype_v fiB_4 = sfomega[1] * 
                                       chi[im];
      const cxtype_v fiB_5 = sfomega[1] * 
                                       chi[ip];
    ...
    else
    {
      ...
      volatile fptype_sv p0p3 = fpmax( pvec0 + 
                                            pvec3, 0 ); // volatile fixes #736
      ...
      const cxtype_sv chi[2] = { cxmake( sqp0p3, 0. ),
                                 ( sqp0p3 == 0. ? cxmake( -(fptype)nhel * 
                                                                       fpsqrt( 2. * 
                                                                       pvec0 ), 0. ) : cxmake( (fptype)nh * 
                                                                       pvec1, pvec2 ) / 
                                                                       sqp0p3 ) };
#endif
      if( nh == 1 )
          ...
          const int ipar )        // input: particle# out of npar
  {
    ...
    const int nh = nhel * 
                       nsf;
    const cxtype_sv sqp0p3 = cxmake( fpsqrt( 2. * 
                                               pvec3 ) * 
                                               (fptype)nsf, 0. );
    fi[2] = fi[1];
    if( nh == 1 )
          ...
          const int ipar )        // input: particle# out of npar
  {
    ...
    const int nh = nhel * 
                       nsf;
    const cxtype_sv chi = cxmake( -(fptype)nhel * 
                                               fpsqrt( -2. * 
                                               pvec3 ), 0. );
    fi[3] = cxzero_sv();
    fi[4] = cxzero_sv();
          ...
          const int ipar )        // input: particle# out of npar
  {
    ...
    const fptype_sv sqp0p3 = fpsqrt( pvec0 + 
                                          pvec3 ) * 
                                          (fptype)nsf;
    const cxtype_sv chi0 = cxmake( sqp0p3, 0. );
    const cxtype_sv chi1 = cxmake( (fptype)nh * 
                                             pvec1 / 
                                             sqp0p3, pvec2 / 
                                             sqp0p3 );
    if( nh == 1 )
          ...
          const int ipar )        // input: particle# out of npar
  {
    ...
    if( vmass != 0. )
    {
      const int nsvahl = nsv * 
                            abs( hel );
      const fptype hel0 = 1. - 
                            abs( hel );
#ifndef MGONGPU_CPPSIMD
      const fptype_sv pt2 = ( pvec1 *
                                   pvec1 ) + 
                                   ( pvec2 * 
                                   pvec2 );
      const fptype_sv pp = fpmin( pvec0, fpsqrt( pt2 + 
                                                    ( pvec3 * 
                                                    pvec3 ) ) );
      ...
      else
      {
        ...
        const fptype emp = pvec0 / 
                                vmass / 
                                pp; // workaround for FPE #1011
        ...
        else
        {
          ...
          vc[4] = cxmake( 0., nsvahl * 
                                    ( pvec3 < 0. ? -sqh : sqh ) ); // AV: removed an abs here
        }
      }
#endif
    }
...
#ifndef MGONGPU_CPPSIMD
      const fptype_sv pt = fpsqrt( ( pvec1 * 
                                          pvec1 ) + 
                                          ( pvec2 * 
                                          pvec2 ) );
#endif
          ...
          const int ipar )        // input: particle# out of npar
  {
    ...
    if( fmass != 0. )
    {
      ...
      else
      {
        ...
        const int ip = ( 1 + 
                          nh ) / 
                          2; // NB: Fortran is (3+nh)/2 because omega(2) has indexes 1,2 and not 0,1
        const int im = ( 1 - 
                          nh ) / 
                          2; // NB: Fortran is (3-nh)/2 because omega(2) has indexes 1,2 and not 0,1
        const fptype sfomeg[2] = { sf[0] * 
                                        omega[ip], sf[1] * 
                                        omega[im] };
        const fptype pp3 = fpmax( pp + 
                                    pvec3, 0. );
      ...
      const int imA = ( 1 + 
                         ...
                         2 * 
                         nhel;
      const cxtype foA_2 = imA * 
                              sqm[abs( ipA )];
      const cxtype foA_3 = ipA * 
                              nsf * 
                              sqm[abs( ipA )];
      const cxtype foA_4 = imA * 
                              nsf * 
                              sqm[abs( imA )];
      const cxtype foA_5 = ipA * 
                              sqm[abs( imA )];
      // Branch B: pp != 0.
      const fptype sf[2] = { fptype( 1 + 
                                      nsf + 
      ...
      const cxtype_v chi[2] = { cxmake( fpsqrt( chi0r2 ), 0. ),    // hack: dummy[ieppV] is not used if pp[ieppV]==0
                                ( cxternary( ( pp3 == 0. ),
                                             cxmake( -nh, 0. ),
                                             cxmake( (fptype)nh * pvec1, -pvec2 ) / fpsqrt( 2. * ppDENOM * pp3DENOM ) ) ) }; // hack: dummy[ieppV] is not used if pp[ieppV]==0
      const cxtype_v foB_2 = sfomeg[1] * 
                                      chi[imB];
      const cxtype_v foB_3 = sfomeg[1] * 
                                      chi[ipB];
      const cxtype_v foB_4 = sfomeg[0] * 
                                      chi[imB];
      const cxtype_v foB_5 = sfomeg[0] * 
                                      chi[ipB];
      // Choose between the results from branch A and branch B
      const bool_v mask = ( pp == 0. );
    ...
    else
    {
      ...
      const fptype_sv sqp0p3 = fpternary( ( pvec1 == 0. ) and ( pvec2 == 0. ) and ( pvec3 < 0. ),
                                          ...
                                          fpsqrt( fpmax( pvec0 + 
                                                              ...
                                                              (fptype)nsf );
      const cxtype_sv chi[2] = { cxmake( sqp0p3, 0. ),
                                 ( sqp0p3 == 0. ? cxmake( -nhel, 0. ) * 
                                                                     fpsqrt( 2. * 
                                                                     pvec0 ) : cxmake( (fptype)nh * 
                                                                     pvec1, -pvec2 ) / 
                                                                     sqp0p3 ) };
#endif
      if( nh == 1 )
          ...
          const int ipar )        // input: particle# out of npar
  {
    ...
    const int nh = nhel * 
                       nsf;
    const cxtype_sv csqp0p3 = cxmake( fpsqrt( 2. * 
                                                pvec3 ) * 
                                                (fptype)nsf, 0. );
    fo[3] = cxzero_sv();
    fo[4] = cxzero_sv();
          ...
          const int ipar )        // input: particle# out of npar
  {
    ...
    fo[0] = cxmake( -pvec3 * 
                          (fptype)nsf, pvec3 * 
                          (fptype)nsf ); // remember pvec0 == -pvec3
    fo[1] = cxzero_sv();
    const int nh = nhel * 
                       nsf;
    const cxtype_sv chi1 = cxmake( -nhel, 0. ) * 
                                              fpsqrt( -2. * 
                                              pvec3 );
    if( nh == 1 )
    {
