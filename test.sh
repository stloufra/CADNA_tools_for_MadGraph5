...
compile_and_run () {
    ...
    then
        ...
        do
            ...
            if [ "$3" == "fortran" ]
                then
            # Compile
                { time  make -j12 OPTFLAGS=" -O3" CUDA_HOME="" AVX=none HASCURAND=hasNoCurand USEOPENMP=1 GTESTLIBS=; } 2>> $save_time #append the error of make
                echo   >> $save_time
            ...
            # Compile
                { time  make -j12  OPTFLAGS=" -O3" CUDA_HOME="" AVX=none HASCURAND=hasNoCurand USEOPENMP=1 GTESTLIBS= ; } 2>> $save_time
                echo   >> $save_time
                echo  "run time" >> $save_time
                echo  "run time" 
            # Run
    ...
    then
        ...
        echo  "make time" >> $save_time
        echo  "make time" 
        { time  make -j12 check_cpp.exe OPTFLAGS=" -O0 -g" CUDA_HOME="" AVX=none HASCURAND=hasNoCurand USEOPENMP=1 GTESTLIBS=; } 2>> $save_time
        echo   >> $save_time
    ...
    then
     #optimalization -O0
        ...
        echo  "make time" >> $save_time
        echo  "make time" 
        { time  make -j12 OPTFLAGS=" -O0 -g" AVX=none HASCURAND=hasNoCurand USEOPENMP=1 GTESTLIBS= CUDA_HOME=""; } 2>> $save_time
        echo   >> $save_time
