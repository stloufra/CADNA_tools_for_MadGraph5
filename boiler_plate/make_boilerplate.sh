...
find "$DIR/boilerplate/Parameters_sm.h" -maxdepth 1 -type f -print0 | while IFS= read -r -d '' file; do
    # Absolute path of the file being checked
    ABS="$(readlink -f "$file")"
    ...
    if grep -q '^#include "mgOnGpuConfig.h"' "$file"; then
         echo "Patching: $file"
         sed -i 's|#include "mgOnGpuConfig.h"|#include "typeTraits.h"|' "$file"
    fi
    ...
    if grep -q '^#include "mgOnGpuCxtypes.h"' "$file"; then
         echo "Removing mgOnGpuCxtypes.h include from: $file"
         sed -i '/^#include "mgOnGpuCxtypes.h"/d' "$file"
    fi

    if grep -q '^#include "constexpr_math.h"' "$file"; then
         echo "Removing constexpr_math.h include from: $file"
         sed -i '/^#include "constexpr_math.h"/d' "$file"
    fi
