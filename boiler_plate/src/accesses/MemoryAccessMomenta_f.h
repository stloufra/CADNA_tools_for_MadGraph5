...
#ifndef MemoryAccessMomenta_f_H
#define MemoryAccessMomenta_f_H 1
...
#endif
{
  ...
  class MemoryAccessMomentaBase_f //_AOSOAv1
  {
  public:
    ...
    // -----------------------------------------------------------------------------------------------
    // --- GPUs: neppM is best set to a power of 2 times the number of fptype_f's in a 32-byte cacheline
    ...
    // -----------------------------------------------------------------------------------------------
    //static constexpr int neppM = 64/sizeof(fptype_f); // 2x 32-byte GPU cache lines (512 bits): 8 (DOUBLE) or 16 (FLOAT)
    static constexpr int neppM = 32/sizeof(fptype_f); // (DEFAULT) 32-byte GPU cache line (256 bits): 4 (DOUBLE) or 8 (FLOAT)
    //static constexpr int neppM = 1; // *** NB: this is equivalent to AOS *** (slower: 1.03E9 instead of 1.11E9 in eemumu)
#else
    // -----------------------------------------------------------------------------------------------
    // --- CPUs: neppM is best set equal to the number of fptype_f's (neppV) in a vector register
    ...
    //static constexpr int neppM = 64/sizeof(fptype_f); // maximum CPU vector width (512 bits): 8 (DOUBLE) or 16 (FLOAT)
    //static constexpr int neppM = 32/sizeof(fptype_f); // lower CPU vector width (256 bits): 4 (DOUBLE) or 8 (FLOAT)
  ...
  class MemoryAccessMomenta_f : public MemoryAccessMomentaBase_f
  {
  public:

    // Locate an event record (output) in a memory buffer (input) from the given event number (input)
    // [Signature (non-const) ===> fptype_f* ieventAccessRecord( fptype_f* buffer, const int ievt ) <===]
    static constexpr auto ieventAccessRecord = MemoryAccessHelper<MemoryAccessMomentaBase_f, fptype_f>::ieventAccessRecord;

    // Locate an event record (output) in a memory buffer (input) from the given event number (input)
    // [Signature (const) ===> const fptype_f* ieventAccessRecordConst( const fptype_f* buffer, const int ievt ) <===]
    static constexpr auto ieventAccessRecordConst = MemoryAccessHelper<MemoryAccessMomentaBase_f, fptype_f>::ieventAccessRecordConst;

    // Locate a field (output) of an event record (input) from the given field indexes (input)
    // [Signature (non-const) ===> fptype_f& decodeRecord( fptype_f* buffer, const int ipar, const int ipar ) <===]
    static constexpr auto decodeRecordIp4Ipar = MemoryAccessHelper<MemoryAccessMomentaBase_f, fptype_f>::decodeRecord;

    // Locate a field (output) of an event record (input) from the given field indexes (input)
    // [Signature (const) ===> const fptype_f& decodeRecordConst( const fptype_f* buffer, const int ipar, const int ipar ) <===]
    static constexpr auto decodeRecordIp4IparConst =
      MemoryAccessHelper<MemoryAccessMomentaBase_f, fptype_f>::template decodeRecordConst<int, int>;

    // Locate a field (output) in a memory buffer (input) from the given event number (input) and the given field indexes (input)
    // [Signature (non-const) ===> fptype_f& ieventAccessIp4Ipar( fptype_f* buffer, const ievt, const int ipar, const int ipar ) <===]
    static constexpr auto ieventAccessIp4Ipar =
      MemoryAccessHelper<MemoryAccessMomentaBase_f, fptype_f>::template ieventAccessField<int, int>;

    // Locate a field (output) in a memory buffer (input) from the given event number (input) and the given field indexes (input)
    // [Signature (const) ===> const fptype_f& ieventAccessIp4IparConst( const fptype_f* buffer, const ievt, const int ipar, const int ipar ) <===]
    // DEFAULT VERSION
    static constexpr auto ieventAccessIp4IparConst =
      MemoryAccessHelper<MemoryAccessMomentaBase_f, fptype_f>::template ieventAccessFieldConst<int, int>;
    ...
    ieventAccessIp4IparConst( const fptype_f* buffer,
                                            const int ievt,
                                            const int ip4,
                                            const int ipar )
    {
      const fptype_f& out = MemoryAccessHelper<MemoryAccessMomentaBase_f, fptype_f>::template ieventAccessFieldConst<int, int>( buffer, ievt, ip4, ipar );
  ...
  class KernelAccessMomenta_f
  {
  public:

    // Expose selected functions from MemoryAccessMomenta_f
    static constexpr auto ieventAccessRecordConst = MemoryAccessMomenta_f::ieventAccessRecordConst;
    ...
    // DEBUG VERSION WITH PRINTOUTS
    static   inline const fptype_f&
    kernelAccessIp4IparConst_s( const fptype_f* buffer,
                                const int ip4,
                                const int ipar )
    {
      const fptype_f& out = KernelAccessHelper<MemoryAccessMomentaBase_f, fptype_f, onDevice>::template kernelAccessFieldConst<int, int>( buffer, ip4, ipar );
    ...
    */
    ...
    // Locate a field (output) in a memory buffer (input) from a kernel event-indexing mechanism (internal) and the given field indexes (input)
    // [Signature (const, SCALAR OR VECTOR) ===> fptype_f_sv kernelAccessIp4IparConst( const fptype_f* buffer, const int ipar, const int ipar ) <===]
    ...
    static   inline fptype_f_sv
    kernelAccessIp4IparConst( const fptype_f* buffer,
                              const int ip4,
                              const int ipar )
    {
      const fptype_f& out = kernelAccessIp4IparConst_s( buffer, ip4, ipar );
#ifndef MGONGPU_CPPSIMD
      return out;
#else
      constexpr int neppM = MemoryAccessMomenta_fBase::neppM;
      ...
      if constexpr( useContiguousEventsIfPossible && ( neppM >= neppV ) && ( neppM % neppV == 0 ) )
      {
        ...
        if constexpr( skipAlignmentCheck )
        {
          ...
          return mg5amcCpu::fptype_fvFromAlignedArray( out ); // use reinterpret_cast
        }
        else if( (size_t)( buffer ) % mgOnGpu::cppAlign == 0 )
        {
          ...
          return mg5amcCpu::fptype_fvFromAlignedArray( out ); // SIMD bulk load of neppV, use reinterpret_cast
        }
        else
        {
          ...
          return mg5amcCpu::fptype_fvFromUnalignedArray( out ); // SIMD bulk load of neppV, do not use reinterpret_cast (fewer SIMD operations)
        }
      }
      else
      {
        ...
        auto decoderIeppv = [buffer, ip4, ipar]( int ieppV )
          -> const fptype_f&
        { return MemoryAccessMomenta_f::ieventAccessIp4IparConst( buffer, ievt0 + ieppV, ip4, ipar ); };
        return mg5amcCpu::fptype_fvFromArbitraryArray( decoderIeppv ); // iterate over ieppV in neppV (no SIMD)
      }
#endif
    }
