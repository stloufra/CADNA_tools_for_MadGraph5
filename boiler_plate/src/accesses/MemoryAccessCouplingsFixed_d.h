...
#ifndef MemoryAccessCouplingsFixed_d_H
#define MemoryAccessCouplingsFixed_d_H 1
...
#endif
{
  ...
  class MemoryAccessCouplingsFixedBase_d //_Sv1
  {
  public:
    ...
    iicoupAccessBufferConst( const fptype_d* buffer, // input "super-buffer": in practice, the cIPC global array
                             const int iicoup )
    {
      constexpr int ix2 = 0;
  ...
  class KernelAccessCouplingsFixed_d
  {
  public:

    // Expose selected functions from MemoryAccessCouplingsFixedBase
    static constexpr auto iicoupAccessBufferConst = MemoryAccessCouplingsFixedBase_d::iicoupAccessBufferConst;
    ...
    kernelAccessConst( const fptype_d* buffer )
    {
      ...
      const fptype_d r_sv = fptype_d{ 0 } + buffer[0];
      const fptype_d i_sv = fptype_d{ 0 } + buffer[1];
      return cxmake<fptype_d>( r_sv, i_sv ); // ugly but effective
    }
  };

  //----------------------------------------------------------------------------

  typedef KernelAccessCouplingsFixed_d<false> HostAccessCouplingsFixed_d;
  typedef KernelAccessCouplingsFixed_d<true> DeviceAccessCouplingsFixed_d;
