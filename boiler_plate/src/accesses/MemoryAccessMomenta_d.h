...
#ifndef MemoryAccessMomenta_d_H
#define MemoryAccessMomenta_d_H 1
...
#endif
{
  ...
  class MemoryAccessMomentaBase_d //_AOSOAv1
  {
  public:
    ...
    // -----------------------------------------------------------------------------------------------
    // --- GPUs: neppM is best set to a power of 2 times the number of fptype_d's in a 32-byte cacheline
    ...
    // -----------------------------------------------------------------------------------------------
    //static constexpr int neppM = 64/sizeof(fptype_d); // 2x 32-byte GPU cache lines (512 bits): 8 (DOUBLE) or 16 (FLOAT)
    static constexpr int neppM = 32/sizeof(fptype_d); // (DEFAULT) 32-byte GPU cache line (256 bits): 4 (DOUBLE) or 8 (FLOAT)
    //static constexpr int neppM = 1; // *** NB: this is equivalent to AOS *** (slower: 1.03E9 instead of 1.11E9 in eemumu)
#else
    // -----------------------------------------------------------------------------------------------
    // --- CPUs: neppM is best set equal to the number of fptype_d's (neppV) in a vector register
    ...
    //static constexpr int neppM = 64/sizeof(fptype_d); // maximum CPU vector width (512 bits): 8 (DOUBLE) or 16 (FLOAT)
    //static constexpr int neppM = 32/sizeof(fptype_d); // lower CPU vector width (256 bits): 4 (DOUBLE) or 8 (FLOAT)
  ...
  class MemoryAccessMomenta_d : public MemoryAccessMomentaBase_d
  {
  public:

    // Locate an event record (output) in a memory buffer (input) from the given event number (input)
    // [Signature (non-const) ===> fptype_d* ieventAccessRecord( fptype_d* buffer, const int ievt ) <===]
    static constexpr auto ieventAccessRecord = MemoryAccessHelper<MemoryAccessMomentaBase_d, fptype_d>::ieventAccessRecord;

    // Locate an event record (output) in a memory buffer (input) from the given event number (input)
    // [Signature (const) ===> const fptype_d* ieventAccessRecordConst( const fptype_d* buffer, const int ievt ) <===]
    static constexpr auto ieventAccessRecordConst = MemoryAccessHelper<MemoryAccessMomentaBase_d, fptype_d>::ieventAccessRecordConst;

    // Locate a field (output) of an event record (input) from the given field indexes (input)
    // [Signature (non-const) ===> fptype_d& decodeRecord( fptype_d* buffer, const int ipar, const int ipar ) <===]
    static constexpr auto decodeRecordIp4Ipar = MemoryAccessHelper<MemoryAccessMomentaBase_d, fptype_d>::decodeRecord;

    // Locate a field (output) of an event record (input) from the given field indexes (input)
    // [Signature (const) ===> const fptype_d& decodeRecordConst( const fptype_d* buffer, const int ipar, const int ipar ) <===]
    static constexpr auto decodeRecordIp4IparConst =
      MemoryAccessHelper<MemoryAccessMomentaBase_d, fptype_d>::template decodeRecordConst<int, int>;

    // Locate a field (output) in a memory buffer (input) from the given event number (input) and the given field indexes (input)
    // [Signature (non-const) ===> fptype_d& ieventAccessIp4Ipar( fptype_d* buffer, const ievt, const int ipar, const int ipar ) <===]
    static constexpr auto ieventAccessIp4Ipar =
      MemoryAccessHelper<MemoryAccessMomentaBase_d, fptype_d>::template ieventAccessField<int, int>;

    // Locate a field (output) in a memory buffer (input) from the given event number (input) and the given field indexes (input)
    // [Signature (const) ===> const fptype_d& ieventAccessIp4IparConst( const fptype_d* buffer, const ievt, const int ipar, const int ipar ) <===]
    // DEFAULT VERSION
    static constexpr auto ieventAccessIp4IparConst =
      MemoryAccessHelper<MemoryAccessMomentaBase_d, fptype_d>::template ieventAccessFieldConst<int, int>;
    ...
    ieventAccessIp4IparConst( const fptype_d* buffer,
                                            const int ievt,
                                            const int ip4,
                                            const int ipar )
    {
      const fptype_d& out = MemoryAccessHelper<MemoryAccessMomentaBase_d, fptype_d>::template ieventAccessFieldConst<int, int>( buffer, ievt, ip4, ipar );
  ...
  class KernelAccessMomenta_d
  {
  public:

    // Expose selected functions from MemoryAccessMomenta_d
    static constexpr auto ieventAccessRecordConst = MemoryAccessMomenta_d::ieventAccessRecordConst;
    ...
    // DEBUG VERSION WITH PRINTOUTS
    static   inline const fptype_d&
    kernelAccessIp4IparConst_s( const fptype_d* buffer,
                                const int ip4,
                                const int ipar )
    {
      const fptype_d& out = KernelAccessHelper<MemoryAccessMomentaBase_d, fptype_d, onDevice>::template kernelAccessFieldConst<int, int>( buffer, ip4, ipar );
    ...
    */
    ...
    kernelAccessIp4IparConst( const fptype_d* buffer,
                              ...
                              const int ipar )
    {
      const fptype_d& out = kernelAccessIp4IparConst_s( buffer, ip4, ipar );
#ifndef MGONGPU_CPPSIMD
      return out;
#else
      constexpr int neppM = MemoryAccessMomenta_dBase::neppM;
      ...
      if constexpr( useContiguousEventsIfPossible && ( neppM >= neppV ) && ( neppM % neppV == 0 ) )
      {
        ...
        if constexpr( skipAlignmentCheck )
        {
          ...
          return mg5amcCpu::fptype_dvFromAlignedArray( out ); // use reinterpret_cast
        }
        ...
        else
        {
          ...
          return mg5amcCpu::fptype_dvFromUnalignedArray( out ); // SIMD bulk load of neppV, do not use reinterpret_cast (fewer SIMD operations)
        }
      }
      else
      {
        ...
        auto decoderIeppv = [buffer, ip4, ipar]( int ieppV )
          -> const fptype_d&
        { return MemoryAccessMomenta_d::ieventAccessIp4IparConst( buffer, ievt0 + ieppV, ip4, ipar ); };
        return mg5amcCpu::fptype_dvFromArbitraryArray( decoderIeppv ); // iterate over ieppV in neppV (no SIMD)
      }
#endif
    }
